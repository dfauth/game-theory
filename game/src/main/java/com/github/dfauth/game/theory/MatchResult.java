...
public class MatchResult {
    ...
    public static Function<List<MatchResult>, MatchResult> reduce = l -> l.stream().reduce(MatchResult::add).orElseThrow(() -> new IllegalArgumentException("No results provided"));
    ...
    public MatchResult(Map<String, Score> map) {
        this.map.putAll(map);
    }

    public MatchResult add(MatchResult r) {
        r.forEach((k,v) -> {
